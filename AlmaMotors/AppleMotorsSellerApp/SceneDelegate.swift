...
//
//  Created by Didar Bakhitzhanov on 5/6/20.
...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if defaults.bool(forKey: kCURRENTUSER) {
            ...
            let tabBar = storyboard.instantiateViewController(withIdentifier: "WelcomeViewController")
            let rootNC = UINavigationController(rootViewController: tabBar)
            self.window?.rootViewController = rootNC
        }
